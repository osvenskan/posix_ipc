...
Windows doesn't provide any POSIX IPC support by default. It can be provided by other packages. It's been over a decade since I tested `posix_ipc`'s compatibility with [Cygwin](http://www.cygwin.com/), but it worked then and probably still does. There may be other options to get POSIX IPC support under Windows (perhaps the Windows Subsystem for Linux), too. I haven't used Windows in many years, so I'm out of touch with that ecosystem and can't provide advice on it.
